...
class Linpack {
    ...
    public static class PrinterBase {
        public void print (String s) { }
    }
    
    public static class PCJPrinter extends PrinterBase {
        public void print (String s) { PCJ.log(s); }
    }
    
    public static class StdOutPrinter extends PrinterBase {
        public void print (String s) { System.out.println(s); }
    }
    
    public static void main(String[] args) {
        Linpack l = new Linpack();
        l.run_benchmark(new StdOutPrinter());
    }
    ...
    double second() {
        if (second_orig == -1) {
            second_orig = mxBean.getCurrentThreadCpuTime();//System.currentTimeMillis();
        }
        return (mxBean.getCurrentThreadCpuTime() - second_orig) * 1e-9;
        //return (System.currentTimeMillis() - second_orig) / 1000;
    }

    public double run_benchmark () {
        return run_benchmark(new PrinterBase());
    }
    
    public double run_benchmark(PrinterBase printer) {
        ...
        final int NN = 4500;
        double a[][] = new double[NN][NN + 1];
        double b[] = new double[NN];
        double x[] = new double[NN];
        double cray, ops, total = 0, norma = 0, normx = 0;
        double resid = 0, time;
        double kf;
        int i, ntimes, info, lda, ldaa, kflops;
        int ipvt[] = new int[NN];
        ...
        printer.print("Mflops/s: " + mflops_result
                    + "  Time: " + time_result + " secs"
                    + "  Norm Res: " + residn_result
                    + "  Precision: " + eps_result);
